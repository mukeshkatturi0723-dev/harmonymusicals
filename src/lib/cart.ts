...
export function getCart(): StoredItem[] {
  try { return JSON.parse(localStorage.getItem(KEY) || "[]") as StoredItem[]; } catch { return []; }
}
...
export function useCart() {
  ...
  const items: CartItem[] = stored.map(item => { const product = { ...item, description: "", features: [], category: "Guitars & Basses" as const, stock: 999 }; return { product, quantity: item.quantity }; });
