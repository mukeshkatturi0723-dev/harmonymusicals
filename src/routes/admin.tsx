...
function AdminDashboard() {
  ...
  return <main className="min-h-screen bg-background px-5 py-12 md:px-8 md:py-20"><div className="mx-auto max-w-[1180px]">
    <div className="flex flex-col gap-4 border-b border-border pb-8 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Harmony Musicals</p><h1 className="mt-2 font-display text-4xl md:text-5xl">Admin dashboard</h1><p className="mt-3 text-sm text-muted-foreground">Manage catalogue pricing and inventory.</p></div><div className="flex gap-4"><button onClick={() => { resetCatalog(); setItems(getProducts()); }} className="text-sm text-muted-foreground underline">Reset demo data</button><Link to="/instruments" className="text-sm underline">View storefront →</Link></div></div>
    ...
    <div className="mt-6 border border-border bg-sand/30 p-5 text-sm"><p className="font-medium">Catalog persistence</p><p className="mt-1 text-muted-foreground">Price and stock changes are stored locally and shared between the storefront and admin on this browser. Cloud persistence and authenticated admin access are the next production layer.</p></div>
  </div></main>;
}
