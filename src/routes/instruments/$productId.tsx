...
function ProductDetails() {
  ...
  const related = items.filter((item) => item.category === product.category && item.id !== product.id);
  const add = () => { if (product.stock < 1) return; addToCart(product); setAdded(true); window.setTimeout(() => setAdded(false), 1400); };
  const buy = () => { if (product.stock < 1) return; addToCart(product); navigate({ to: "/checkout" }); };
